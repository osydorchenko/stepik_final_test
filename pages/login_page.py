...
class LoginPage(BasePage):
    def register_new_user(self, email, password):
        ...
        assert self.is_element_present(*LoginPageLocators.REGISTER_EMAIL), 'No email-field on page'
        email_field = self.browser.find_element(*LoginPageLocators.REGISTER_EMAIL)
        email_field.send_keys(self.email)
        assert self.is_element_present(*LoginPageLocators.REGISTER_PASSWORD1), 'No password1-field on page'
        password1_field = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD1)
        password1_field.send_keys(self.password)
        assert self.is_element_present(*LoginPageLocators.REGISTER_PASSWORD2), 'No password2-field on page'
        password2_field = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD2)
        password2_field.send_keys(self.password)
        assert self.is_element_present(*LoginPageLocators.REGISTER_BUTTON), 'No register-button on page'
